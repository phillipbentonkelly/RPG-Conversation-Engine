...
(function(){
	...
	RPGcEngine.prototype = {
		...
		handleDialogOnFinishStage: function( dataObj, targetObj ){
			var thisRef = this;
			var currDialogArr = $('.rpgcEngine_dialogBox');

			if(typeof(dataObj.onFinish.type) != 'undefined' && dataObj.onFinish.type.length > 0){
				switch(dataObj.onFinish.type){
					case "onFinish":
						currDialogArr.not(targetObj).animate({opacity: 0}, 400, function(){
							currDialogArr.not(targetObj).remove();
						});
					...
					case "userEvent":
						thisRef.stageObj.bind('click', function(){
							thisRef.stageObj.unbind('click');

							currDialogArr.not(targetObj).animate({opacity: 0}, 400, function(){
								currDialogArr.not(targetObj).remove();
							});

							var handleDialogOnFinishStageTimeout = setTimeout(function(){
								thisRef.removeDialogBubble( targetObj, dataObj, dataObj.onFinish.advance );
							}, dataObj.onFinish.delay);
						});
					default: ""
